...
- **env.prepend** prepends colon-separated values to existing variables; supports `$VAR` expansion against the already-built env (including forced vars like `$HOME`); applied after forced vars
- **Reserved keys** (HOME, XDG_CONFIG_HOME, XDG_CACHE_HOME, XDG_DATA_HOME, XDG_STATE_HOME, TMPDIR, SILO_ROOT, SILO_HOST_HOME) cannot appear in env.set, env.prepend, or secrets.items
